...
import { Categoria, FiltroPecas, Fornecedor, FormularioPeca, Peca, RespostaEscrita } from "./tipos.js";

let pecas: Peca[] = [];
let categorias: Categoria[] = [];
let fornecedores: Fornecedor[] = [];

const filtroCompleto: FiltroPecas = {
    busca: "",
    categoria: 0,
    situacao: "",
    pagina: 1,
...
const lerCampo = (id: string): string => {
    const alvo = campo(id);
    return (alvo) ? alvo.value.trim() : "";
}

const escreverCampo = (id: string, valor: string): void => {
    const alvo = campo(id);
...
const avisar = (mensagem: string, deuCerto: boolean): void => {
...
const tratarResposta = (resposta: RespostaEscrita): boolean => {
    if (resposta.ok) {
...
const botoesDaLinha = (tipo: string, id: number): string => {
    return `
...
const renderizarFornecedores = (): void => {
    let html = "";
...
const limparFormularioPeca = (): void => {
...
const limparFormularioCategoria = (): void => {
    escreverCampo("categoriaId", "0");
    escreverCampo("categoriaNome", "");
}

const limparFormularioFornecedor = (): void => {
...
const editarPeca = (id: number): void => {
    for (const peca of pecas) {
        if (peca.id === id) {
...
const editarFornecedor = (id: number): void => {
    for (const fornecedor of fornecedores) {
        if (fornecedor.id === id) {
...
const executarAcao = async (acao: string, tipo: string, id: number): Promise<void> => {
    try {
        if (acao === "editar") {
            if (tipo === "peca") {
                editarPeca(id);
        ...
        let resposta: RespostaEscrita = { ok: false, erro: "Acao desconhecida." };
...
const ligarLista = (id: string): void => {
